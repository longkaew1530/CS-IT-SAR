...
@foreach(session()->get('getfaculty')  as $faculty)
    @foreach($faculty->course as $valuecourse)
         <li >
            <span>{{$valuecourse['course_name']}}</span>

          @foreach($valuecourse->branch as $valueb)
          <ul id='myid'>
            <li id="{{$valueb['branch_id']}}" class="active" onclick="calc2()"><a href="">{{$valueb['name']}}</a></li>
          </ul>
          @endforeach
        </li>
        @endforeach
